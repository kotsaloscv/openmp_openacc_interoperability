...
#define _PRAGMA_FOR_CUR_ACC_LOOP_ _Pragma("acc parallel loop present(_ni[0:_cntml_actual], _nt_data[0:_nt->_ndata], _p[0:_cntml_padded*_psize], _ppvar[0:_cntml_padded*_ppsize], _vec_v[0:_nt->end], _vec_d[0:_nt->end], _vec_rhs[0:_nt->end], _nt[0:1] _thread_present_) copy(rhs_compare[0:_cntml_actual])")
//#define _PRAGMA_FOR_CUR_ACC_LOOP_ _Pragma("acc parallel loop present(_ni[0:_cntml_actual], _nt_data[0:_nt->_ndata], _p[0:_cntml_padded*_psize], _ppvar[0:_cntml_padded*_ppsize], _vec_v[0:_nt->end], _vec_d[0:_nt->end], _vec_rhs[0:_nt->end], _nt[0:1] _thread_present_)")
...
_cntml_padded = _ml->_nodecount_padded;
_thread = _ml->_thread;
double *_vec_rhs = (double*)_nt->_actual_rhs;
...
double * _vec_v = _nt->_actual_v;
int stream_id = _nt->stream_id;
double *rhs_compare = new double[_cntml_actual];
#pragma acc enter data copyin(rhs_compare[0:_cntml_actual])
#if LAYOUT == 1 /*AoS*/
for (_iml = 0; _iml < _cntml_actual; ++_iml) {
 _p = _ml->_data + _iml*_psize; _ppvar = _ml->_pdata + _iml*_ppsize;
#elif LAYOUT == 0 /*SoA*/
...
for (;;) { /* help clang-format properly indent */
 ...
 _PRCELLSTATE_G
	...
	_vec_d[_nd_idx] += _g;
   rhs_compare[_iml] = _vec_rhs[_nd_idx];
 
}
#pragma acc update host(_vec_rhs[0:_nt->end])
#pragma acc compare(rhs_compare[0:_cntml_actual])
//#pragma acc compare(_vec_rhs[0:_nt->end])
#pragma acc exit data copyout(rhs_compare[0:_cntml_actual])
//#pragma acc compare(rhs_compare[0:_cntml_actual]) 
//#pragma acc compare(_vec_rhs[0:_nt->end])
//acc_compare(_vec_rhs, _nt->end);
}
